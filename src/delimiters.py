...
def delimit_inline_nodes(text: str, patterns=list[str]) -> list[TextNode]:
    combined_pattern = "|".join(f"({p})" for p in patterns) if len(patterns) > 1 else patterns[0]
    nodes = []
...
def delimit_nodes(text: str, patterns=list[str]) -> list[TextNode]:
    ...
    return [TextNode(text, TextType.TEXT, span=(0, len(text)))]


def delimit_blocks(text: str, pattern: str) -> list[str]:
    return list(filter(lambda x: x != "", re.split(pattern, text.strip(), flags=re.DOTALL)))
